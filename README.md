...
| editedCells | 编辑过的单元格 | Array | | [] |
| actEditCell | 激活编辑的单元格 | Object | | {row:'',cell:''} |
| data | 表格数据 | Array | | [] |
...
| url | 数据源 | String | | |
| params | 参数 | Array | | [] |
| handleParams | 参数处理 | Function(入参，分页数据) 返回：新参数 | | | 
| handle | 发送请求前的参数处理 | Function(参数) 返回：新参数 | | | 
| fail | 请求失败事件 | Function（错误） | | |
| done | 请求成功 | Function（响应）返回：表格数据 | | | 
